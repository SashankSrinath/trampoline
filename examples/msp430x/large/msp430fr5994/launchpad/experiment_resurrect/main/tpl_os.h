...
/*--------------------------------------------------------------------------*
 * checkpoint system calls.
 * corresponding kernel implementation is in files
 * tpl_chkpt_checkpoint_kernel.h and tpl_chkpt_checkpoint_kernel.c
 *--------------------------------------------------------------------------*/

/*
 * Hibernate
 *
 * The user calls this system service to checkpoint the current status of 
 * the OS. 
 */
FUNC(void, OS_CODE) Hibernate(void);


/*--------------------------------------------------------------------------*
 * resurrect system calls.
 * corresponding kernel implementation is in files
 * tpl_resurrect_kernel.h and tpl_resurrect_kernel.c
 *--------------------------------------------------------------------------*/

/*
 * StartResurrect
 *
 * The user can call this system service to start the os with resurrect 
 * in a specific mode. 
 *
 * mode: The active application mode 
 */
FUNC(void, OS_CODE) StartResurrect(
  CONST(AppModeType, AUTOMATIC) mode);

/*
 * TerminateStepResurrect
 *
 * Terminate the current activity and start another one if possible. 
 */
FUNC(void, OS_CODE) TerminateStepResurrect(void);

/*
 * SetNbAlarmActivation
 *
 * Set the number of activation of alarm. 
 *
 * alarm_id: The identifier of the alarm 
 *
 * set_nb_activation: Number of activation of alarm. 
 */
FUNC(void, OS_CODE) SetNbAlarmActivation(
  CONST(AlarmType, AUTOMATIC) alarm_id,
  VAR(int, AUTOMATIC) set_nb_activation);

/**
 * Re-Starts Trampoline after a checkpoint.
 *
 */
extern FUNC(void, OS_CODE) RestartOS();
